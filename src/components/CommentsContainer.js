...
class CommentsContainers extends React.Component {
  ...
  handleCommentSubmit = (validateRules, setFieldsValue) => {
    return (event) => {
      ...
      validateRules((error, values) => {
        ...
        return postNewComment(this.props.postId, values)
          .then(({ data }) => {
            const comments = [].concat(this.state.comments, [data]);
  ...
  handleCommentUpdate = (validateRules, setFieldsValue, comment) => {
    return (event) => {
      event.preventDefault();
      this.setState({ loading: true });
      validateRules((error, values) => {
